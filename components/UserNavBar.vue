...
<template>
  <nav
    class="sticky top-0 min-h-[calc(100vh-4rem)] flex flex-col text-lg col-span-1  text-right text-primary-200 border-r-2 border-gray-200 dark:border-primary-400 px-4"
  >
    <NuxtLink v-slot="{ isActive }" to="/user/overview" title="Overview">
      <div class="mt-5" :class="[isActive ? 'text-secondary-100' : 'hover:text-primary-100']">
        <span class="pr-1 hidden md:inline">Overview</span>
        <Icon class="mr-2 text-4xl md:text-xl" name="material-symbols:folder-copy" />
      </div>
    </NuxtLink>
    <NuxtLink v-slot="{ isActive }" to="/user/wallet" title="Wallet">
      <div class="mt-5" :class="[isActive ? 'text-secondary-100' : 'hover:text-primary-100']">
    ...
    <a href="#" title="Logout" class="mt-5 hover:text-primary-200 " @click="user.logout()">
      <span class="pr-1 hidden md:inline">Log out</span>
