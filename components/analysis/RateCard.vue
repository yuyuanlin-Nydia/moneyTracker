...
function getRateText(rate: string) {
  if (parseFloat(rate) > 0) return 'Increase';
  else if (parseFloat(rate) < 0) return 'Decrease'
  else return 'Balance'
}
...
function getIcon(rate: string) {
  return parseFloat(rate) > 0 ? 'heroicons-solid:arrow-circle-up'
    : parseFloat(rate) < 0 ? 'heroicons-solid:arrow-circle-down'
...
<template>
  <div v-if="props.item" class="rounded-md border-secondary-100 border bg-primary-400 mb-5 basis-[48%]">
    <div class="font-bold text-2xl  border-secondary-100 border-b p-2">{{ props.item.type }}</div>
    <div class="p-2 flex justify-between items-start">
      ...
      <div class="flex">
        <div :class="['rounded-lg', 'p-1', 'mr-2', 'min-w-[140px]', 'text-center', getRateStyle(props.item.MoM)]">
          <Icon :name="getIcon(props.item.MoM)" class="rounded-full p-1 text-2xl" />
          <span>{{ props.item.MoM }}%</span>
          <span>/MoM</span>
        </div>
        <div :class="['rounded-lg', 'p-1', 'min-w-[140px]', 'text-center', getRateStyle(props.item.YoY)]">
